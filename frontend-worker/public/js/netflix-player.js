class NetflixIPTVPlayer {
    ...
    resolveDefaultPlaylistType() {
        ...
        if (fromUrl && this.playlists[fromUrl]) {
            return fromUrl;
        }
        const stored = localStorage.getItem('streamflix-preferred-playlist');
        if (stored && this.playlists[stored]) {
            return stored;
        }
        return 'benfica';
    }
    ...
    resolveRelayEndpoint() {
        ...
        const cloudflareDefault = 'https://streamflix-v2-worker.cristianoronaldocr7.workers.dev';
        const raw = fromWindow || fromMeta || cloudflareDefault;
        return raw.replace(/\/+$/, '');
    }
