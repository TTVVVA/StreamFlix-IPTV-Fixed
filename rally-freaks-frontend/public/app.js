...
async function unused_old_playChannel(channel, autoplay, context = {}) {
        maxBufferLength: 30,
        maxMaxBufferLength: 60,
        backBufferLength: 30,
        enableWorker: false,
        fragLoadingTimeOut: state.androidRuntime ? 45000 : 20000,
        manifestLoadingTimeOut: state.androidRuntime ? 8000 : 10000,
        levelLoadingTimeOut: state.androidRuntime ? 8000 : 10000,
        manifestLoadingMaxRetry: 0,
        fragLoadingMaxRetry: state.androidRuntime ? 3 : 2,
        startFragPrefetch: false
      });
      state.hls = hlsInstance;

      const hlsCtx = { channelSeq, playSeq, mutationId, remote, correlationId, channel: channel.name };
      const hlsRecovery = { manifest: 0, audio: 0, codec: 0 };

      hlsInstance.on(Hls.Events.MEDIA_ATTACHED, () => {
        logEvent("hls.media_attached", { ...hlsCtx, video: videoSnapshot() });
      });

      hlsInstance.on(Hls.Events.MANIFEST_PARSED, async (_event, data) => {
        if (shouldIgnorePlaybackCallback(hlsInstance, channelSeq)) {
          await logStalePlaybackCallback("MANIFEST_PARSED", hlsCtx, hlsInstance);
          return;
        }
        refs.videoTime.classList.remove("buffering");
        refs.playerCard.classList.add("has-video");
        setDiag(refs.streamDiag, "ok", "Stream: manifesto OK");
        showToast("Stream preparado.", `diag: ${channel.name}`);
        await logEvent("hls.manifest_parsed", {
          ...hlsCtx,
          levels: data?.levels?.length,
          firstLevel: data?.firstLevel,
          video: videoSnapshot()
        });
        console.log("[HLS] manifest parsed, a tentar play...");
        if (autoplay || state.userInteracted) {
          // Garantir muted antes do play() â€” evita Promise pending silenciosa no Discord WebView.
          if (!refs.streamVideo.muted) {
            refs.streamVideo.muted = true;
            state.autoMutedForAndroid = true;
          }
          await attemptVideoPlay("hls_manifest_parsed", { ...hlsCtx, _hlsInstance: hlsInstance });
        } else {
          state.pendingUserPlayContext = { reason: "manifest_ready_wait_user_gesture", ...hlsCtx };
          setDiag(refs.streamDiag, "warn", "Toque em Ã¢â€“Â¶ para reproduzir");
          await logEvent("video.play_skip", { ...hlsCtx, reason: "autoplay_false_no_user_gesture", userInteracted: state.userInteracted, androidRuntime: state.androidRuntime });
        }
      });

      hlsInstance.on(Hls.Events.ERROR, async (_event, data) => {
        if (shouldIgnorePlaybackCallback(hlsInstance, channelSeq)) {
          await logStalePlaybackCallback("ERROR", hlsCtx, hlsInstance);
          return;
        }

        const fatality = data?.fatal ? "fatal" : "non-fatal";
        const status = data?.response?.code || data?.networkDetails?.status || "";
        const details = String(data?.details || "");
        setDiag(refs.streamDiag, data?.fatal ? "error" : "warn", `HLS: ${fatality}${status ? ` ${status}` : ""}`);
        await logEvent("hls.error", {
          ...hlsCtx,
          ...
          networkDetails: data?.networkDetails
            ? {
                ...
                url: data.networkDetails.url
                  ? redactedUrlForLog(data.networkDetails.url)
                  : undefined
              }
            : undefined,
          responseUrl: data?.response?.url ? redactedUrlForLog(data.response.url) : undefined,
          video: videoSnapshot()
        });

        if (!data?.fatal) return;

        const canRetryManifest = /manifestLoadError|manifestLoadTimeOut/i.test(details) && hlsRecovery.manifest < 1;
        const canRetryAudio = /audioTrackLoadError|audioTrackLoadTimeOut/i.test(details) && hlsRecovery.audio < 1;
        const canRecoverCodec = /bufferAddCodecError|bufferAppendError|bufferAppendingError/i.test(details) && hlsRecovery.codec < 1;

        if (canRetryManifest || canRetryAudio) {
          if (canRetryManifest) hlsRecovery.manifest += 1;
          if (canRetryAudio) hlsRecovery.audio += 1;
          await logEvent("android.hls_retry", {
            ...hlsCtx,
            details,
            status,
            retryKind: canRetryManifest ? "manifest" : "audio_track",
            recovery: hlsRecovery,
            video: videoSnapshot()
          });
          setDiag(refs.streamDiag, "warn", "Stream: retry Android");
          setTimeout(() => {
            if (shouldIgnorePlaybackCallback(hlsInstance, channelSeq)) {
              void logStalePlaybackCallback("RECOVERY_RETRY", hlsCtx, hlsInstance);
              return;
            }
            try {
              if (Number.isFinite(hlsInstance.currentLevel) && hlsInstance.levels?.length > 1) {
                hlsInstance.currentLevel = 0;
              }
              hlsInstance.startLoad(-1);
            } catch (err) {
              void logEvent("android.hls_retry_error", { ...hlsCtx, details, error: serializeError(err), video: videoSnapshot() });
            }
          }, 750);
          return;
        }
        ...
        if (canRecoverCodec) {
          hlsRecovery.codec += 1;
          await logEvent("android.hls_recover_media", {
            ...hlsCtx,
            details,
            status,
            recovery: hlsRecovery,
          ...
          try {
            ...
            await logEvent("android.hls_recover_media_error", { ...hlsCtx, details, error: serializeError(err), video: videoSnapshot() });
          }
        }

        // Distinguir "canal indisponÃ­vel na source" de erro genÃ©rico de stream
        const isChannelUnavailable = status === 502 || status === "502";
        if (isChannelUnavailable) {
          setDiag(refs.streamDiag, "error", "Canal indisponÃ­vel");
          showToast(
            "Canal indisponÃ­vel.",
            "diag: a source deste canal nÃ£o estÃ¡ a responder. Tenta outro canal ou aguarda.",
            "error"
          );
          await logEvent("channel.unavailable", { ...hlsCtx, status, details, video: videoSnapshot() });
        } else {
          showToast("Erro de stream.", `diag: ${status || data?.details || "hls fatal"}`, "error");
        }
      });
      ...
      if (DIAG_HLS_EVENTS) {
        ...
        eventNames.forEach((eventName) => {
          hlsInstance.on(eventName, (_event, data = {}) => {
            if (shouldIgnorePlaybackCallback(hlsInstance, channelSeq)) {
              void logStalePlaybackCallback(eventName, hlsCtx, hlsInstance);
              return;
            }
            logEvent("hls.runtime_event", {
              ...hlsCtx,
              eventName,
              fragSn: data?.frag?.sn,
              ...
              stats: data?.stats ? {
                loadingStart: data.stats.loading?.start,
                loadingEnd: data.stats.loading?.end,
                parsingStart: data.stats.parsing?.start,
                parsingEnd: data.stats.parsing?.end,
                bufferingStart: data.stats.buffering?.start,
                bufferingEnd: data.stats.buffering?.end
              } : undefined,
              video: videoSnapshot()
            });
          });
      ...
      await logEvent("hls.load_source", { ...hlsCtx, proxyUrl: "/proxy?url=<redacted>" });
      hlsInstance.loadSource(proxyUrl);
      hlsInstance.attachMedia(refs.streamVideo);
      return;
    }

    if (refs.streamVideo.canPlayType("application/vnd.apple.mpegurl") || refs.streamVideo.canPlayType("application/x-mpegURL")) {
      if (autoplay) {
        refs.streamVideo.muted = true;
        state.autoMutedForAndroid = true;
        await logEvent("autoplay_muted_bootstrap", { channelSeq, playSeq, correlationId, channel: channel.name, androidRuntime: state.androidRuntime, native: true });
      }
      refs.streamVideo.src = proxyUrl;
      refs.playerCard.classList.add("has-video");
      refs.videoTime.classList.remove("buffering");
      setDiag(refs.streamDiag, "ok", "Stream: HLS nativo");
      await logEvent("hls.native_fallback", { channelSeq, playSeq, mutationId, remote, channel: channel.name, video: videoSnapshot() });
      if (autoplay && state.userInteracted) {
        await attemptVideoPlay("native_hls_autoplay", { channelSeq, playSeq, mutationId, remote, channel: channel.name });
      }
      return;
    }

    throw new Error("HLS nÃƒÆ’Ã‚Â£o suportado neste browser e hls.js indisponÃƒÆ’Ã‚Â­vel.");
  } catch (err) {
    refs.videoTime.classList.remove("buffering");
    setDiag(refs.streamDiag, "error", "Stream: erro");
    showToast("Erro de stream upstream.", `diag: ${err.message}`, "error");
    await logEvent("play.error", { channelSeq, playSeq, mutationId, remote, channel: channel.name, error: serializeError(err), video: videoSnapshot() });
  } finally {
    state.isLoading = false;
    if (state.loadingChannelSeq === channelSeq) state.loadingChannelSeq = 0;
    await logEvent("play.end", { channelSeq, playSeq, mutationId, remote, channel: channel.name, video: videoSnapshot() });
  }
}
